...
if($getFromU->checkPassword($currentPwd)===true){
	if(strlen($newPassword)<6 or strlen($newPassword)>20 ){
		$error['newPassword']="Password must be between 6-20 characters";
	}
	if($rePassword!=$newPassword){
		$error['rePassword']="Passwords do not match";
	}
}else{
	$error['currentPwd']="The password you entered isn't correct";
}
if(!isset($error['currentPwd'])&& !isset($error['newPassword']) && !isset($error['rePassword']) && !isset($error['fields'])){
	$pass=md5($newPassword);
	$getFromU->update('users',$user_id,array('password'=>$pass));
